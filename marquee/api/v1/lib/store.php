...
function marqueeStoreDir(): ?string
{
    ...
    $candidate = rtrim(sys_get_temp_dir(), '/') . '/marquee-api-v1';
...
function marqueeStoreGet(string $key)
{
    if (marqueeStoreUsesApcu()) {
        $ok = false;
        $value = apcu_fetch('marquee:' . $key, $ok);
...
{
    if (marqueeStoreUsesApcu()) {
        return (bool) apcu_store('marquee:' . $key, $value, $ttl);
    }
...
function marqueeStoreIncrement(string $key, int $ttl): ?int
{
    if (marqueeStoreUsesApcu()) {
        $ok = false;
        $count = apcu_inc('marquee:' . $key, 1, $ok, $ttl);
